...
export default () => {
    ...
    return (
        <div className="full-screen flex flex-no-wrap">
            <Navbar />
            <div>
            <h1 className="block text-center text-theme">Forms</h1>
                <div className="flex flex-wrap w-fit pg-t-0-5 pg-b-0-5 mg-t-2 box-fit-nav-bar">
                { relations && relations.map( rel =>
                    <div className="block pg-t-1 pg-l-1 pg-r-1 pg-b-1 hover base-card shadow border-radius-0-5 border-pale" key={ rel.id } onClick={() => {openTab(`/form/${rel.name}`)} }>
